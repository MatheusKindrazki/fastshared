...
### ⚠️ The OG image does not go live with the deploy

`public/og-image.png` is regenerated by `brand/og-image.sh` from
`brand/og-image.html` — change the HTML, never the PNG. But deploying is **not**
enough to make a new card visible, and the reason is easy to mistake for a flaky
CDN:

`/og-image.png` is listed in `APP_PATH_PREFIXES` (`backend/src/index.ts`), so the
apex does **not** proxy it to Pages. The Worker serves it itself, from
`backend/src/routes/assetsPublic.ts`, out of `caches.default` with
`Cache-Control: public, max-age=86400, immutable`. That cache is deliberate — it
keeps Slack/iMessage/Discord unfurlers off the Pages origin on every scrape — but
it means a regenerated card is invisible at the apex for **up to 24 hours per
edge**, and `immutable` tells downstream scrapers not to revalidate, so anything
that unfurls during the window keeps the old card for longer still.

Measured on the 2026-08-17 deploy: Pages served the new bytes immediately while
the apex answered `cf-cache-status: HIT`, `age: 68244`, with the old
`content-length`. Nothing was wrong with the deploy.

So after regenerating, verify both sides and purge:
...
Worth fixing properly at some point: the cache key
(`${SHORT_LINK_HOST}${pathname}`) has no content or version component, so every
future OG change repeats this window. Either shorten `CACHE_MAX_AGE_SECONDS` for
this one asset, key the cache on a build id, or add a purge step to
`web-deploy.yml` — it already holds a `CLOUDFLARE_API_TOKEN`, though whether that
token carries cache-purge scope has not been checked.
